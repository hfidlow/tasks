...
export function isCorrect(question: Question, answer: string): boolean {
    const ans2 = answer.trim();
    const ans3 = ans2.toLowerCase();
    const exp = question.expected.toLowerCase();
    if (ans3 === exp) {
        return true;
    } else {
        return false;
...
export function toShortForm(question: Question): string {
    return JSON.stringify(question.id) + ": " + question.name.slice(0, 10);
}
...
export function toMarkdown(question: Question): string {
    if (question.type === "short_answer_question") {
        const qFormat = "# " + question.name + "\n" + question.body;
        return qFormat;
    } else {
        const opt3 = question.options.map(
            (curr: string): string => "- " + curr
        );
        const qFormat =
            "# " +
            question.name +
...
export function publishQuestion(question: Question): Question {
    if (question.published === true) {
        const newQ = { ...question, published: false };
        return newQ;
    } else {
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    const oldName = oldQuestion.name;
    const newQ = {
        ...oldQuestion,
        id: id,
